...
export function DashboardUserPage() {
  ...
  const { data: viajesActivos } = useQuery({
    queryKey: ['viajes', 'mis-viajes-activos'],
    queryFn: async () => {
  ...
  const { data: todosLosMisViajes } = useQuery({
    queryKey: ['viajes', 'mis-viajes-todos'],
    queryFn: async () => {
          ...
          <div className="bg-gradient-to-br from-[#5A1E5C]/80 to-[#6A2A6E]/80 border-2 border-[#7B97BC]/30 rounded-2xl p-6 shadow-xl hover:shadow-[#7FA2C8]/20 hover:shadow-lg transition-all duration-300 hover:-translate-y-1">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-5xl font-black text-white mt-2">{todosLosMisViajes?.length || 0}</p>
              </div>
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-5xl font-black text-white mt-2">
                  {todosLosMisViajes?.reduce((total: number, viaje: any) => {
                    const km = viaje.kmFinal && viaje.kmInicial ? parseFloat(viaje.kmFinal) - parseFloat(viaje.kmInicial) : 0;
                    return total + km;
                  }, 0).toLocaleString('es-CO') || 0}
                </p>
              </div>
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex-1">
                      ...
                      <div className="flex gap-4 mt-2 text-sm">
                        <span className="text-[#7B97BC]">
                          <span className="text-[#7FA2C8] font-bold">Vehículo:</span> {viaje.vehiculo?.placa} - {viaje.vehiculo?.marca} {viaje.vehiculo?.modelo}
                        </span>
                        <span className="text-[#7B97BC]">
                          <span className="text-[#7FA2C8] font-bold">Iniciado:</span> {new Date(viaje.fechaInicio).toLocaleString('es-CO')}
                        </span>
                      </div>
                      <div className="flex gap-4 mt-2 text-sm">
                        <span className="text-[#7B97BC]">
                          <span className="text-[#7FA2C8] font-bold">KM Inicial:</span> {viaje.kmInicial?.toLocaleString('es-CO')} km
                        </span>
                        <span className="text-[#7B97BC]">
                          <span className="text-[#7FA2C8] font-bold">Gasolina Inicial:</span> {viaje.gasolinaInicial}%
                        </span>
                      </div>
                    </div>
                    <button className="bg-gradient-to-r from-emerald-600 to-emerald-500 hover:from-emerald-500 hover:to-emerald-400 text-white font-bold py-2 px-4 rounded-lg transition-all whitespace-nowrap">
                      Ver Detalles
                    </button>
                  </div>
                </div>
